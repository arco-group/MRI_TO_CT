...
from src.model.PixelDA.generator_model import Generator
...
# --------------------------
if platform == "win32":
    cfg_path = "./configs/pixelda_test.yaml"
else:
    cfg_path = ".../configs/pixelda_test.yaml"
...
for fold in fold_list:
    ...
    # ---- Model G ----
    G = Generator(img_channels=1, latent_dim=latent_dim).to(device)

    # ---- load checkpoint ----
    model_fold_dir = os.path.join(pretrained_path, str(fold))
    ckpt_G = os.path.join(model_fold_dir, cfg["trainer"]["CHECKPOINT_G"])
    ...
    results = util_model.test_pixelda(
        G, test_loader,
