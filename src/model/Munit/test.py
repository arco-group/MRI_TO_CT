...
# --------------------------
if platform == 'win32':
    cfg_file = "./configs/munit_test.yaml"
else:
    cfg_file = ".../configs/munit_test.yaml"
...
for fold in fold_list:
    ...
    dim = mcfg.get("dim", 64)
    n_down = mcfg.get("n_downsample", 2)
    n_res = mcfg.get("n_residual", 3)
    style_dim = mcfg.get("style_dim", 8)
    ...
    CHECKPOINT_ENC1 = os.path.join(pretrained_model_dir, cfg['trainer']['CHECKPOINT_ENC1'])
    CHECKPOINT_DEC2 = os.path.join(pretrained_model_dir, cfg['trainer']['CHECKPOINT_DEC2'])


    load_checkpoint(CHECKPOINT_ENC1, Enc1, map_location=device)
    load_checkpoint(CHECKPOINT_DEC2, Dec2, map_location=device)


    mse_metric  = nn.MSELoss().to(device)
    psnr_metric = PeakSignalNoiseRatio().to(device)

    results = util_model.test_munit(
        Enc1=Enc1, Dec2=Dec2,
