...
from src.model.Dualgan.generator_model import Generator
...
# --------------------------
if platform == 'win32':
    cfg_path = "./configs/dualgan_test.yaml"
else:
    cfg_path = ".../configs/dualgan_test.yaml"
...
pretrained_path = os.path.join(cfg['data']['model_dir'], exp_name)
...
for fold in fold_list:
    ...
    ckpt_gab = os.path.join(pretrained_model_dir, cfg['trainer']['CHECKPOINT_G_AB'])

    util_general.load_checkpoint(ckpt_gab, G_AB, map_location=device)

    # ---- Metric models ----
    mse_metric  = nn.MSELoss().to(device)
    psnr_metric = PeakSignalNoiseRatio().to(device)

    results = util_model.test_dualgan(
        G_AB, loader_test,
...
mse_m   = df_folds["mse"].mean()
mse_std = df_folds["mse"].std(ddof=1)

psnr_m   = df_folds["psnr"].mean()
psnr_std = df_folds["psnr"].std(ddof=1)

vif_m   = df_folds["vif"].mean()
vif_std = df_folds["vif"].std(ddof=1)

ssim_m   = df_folds["ssim"].mean()
ssim_std = df_folds["ssim"].std(ddof=1)

lpips_m   = df_folds["lpips"].mean()
lpips_std = df_folds["lpips"].std(ddof=1)

fid_m   = df_folds["fid"].mean()
fid_std = df_folds["fid"].std(ddof=1)

is_m   = df_folds["is_mean"].mean()
is_std = df_folds["is_mean"].std(ddof=1)
...
for i in range(cv):
    row_out[f"MSE_{i}"] = fcol("mse", i)
for i in range(cv):
    row_out[f"PSNR_{i}"] = fcol("psnr", i)
for i in range(cv):
    row_out[f"VIF_{i}"] = fcol("vif", i)
for i in range(cv):
    row_out[f"SSIM_{i}"] = fcol("ssim", i)
for i in range(cv):
    row_out[f"LPIPS_{i}"] = fcol("lpips", i)
for i in range(cv):
    row_out[f"FID_{i}"] = fcol("fid", i)
for i in range(cv):
    row_out[f"IS_{i}"] = fcol("is_mean", i)
...
print("\n[OK] Salvato Excel (1 riga) in:", excel_path)
