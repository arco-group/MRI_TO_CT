...
for fold in fold_list:
    ...
    latent_dim = cfg['model'].get('latent_dim', 8)  # fallback
    ...
    load_checkpoint(CHECKPOINT_GEN, generator, map_location=device)
    load_checkpoint(CHECKPOINT_ENC, encoder,  map_location=device)
    ...
    print(
        f"[fold {fold}] mse={results['mse']:.6f} psnr={results['psnr']:.4f} "
