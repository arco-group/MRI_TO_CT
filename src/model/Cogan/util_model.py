...
def save_cogan_checkpoint_G(
    ...
    ckpt = {
        ...
        "max_epochs": int(cfg_trainer["max_epochs"]),
    }
    torch.save(ckpt, path)
    print(f"=> [CoGAN] Checkpoint G salvato in {path} (finished={training_finished})")


def save_cogan_checkpoint_D(path, D, opt_D):
    ckpt = {
        "state_dict": D.state_dict(),
        "optimizer": opt_D.state_dict(),
    }
    torch.save(ckpt, path)
    print(f"=> [CoGAN] Checkpoint D salvato in {path}")


def load_cogan_checkpoint_G(path, device):

    print(f"=> [CoGAN] Carico checkpoint G da {path}")
...
def train_cogan(
    ...
    opt_G, opt_D,
    l1, mse_gan,
    model_fold_dir, cfg_trainer, device,
    resume_state=None
):
    ...
    # ---- stato early stopping / resume ----
    if resume_state is not None:
        start_epoch       = resume_state.get("start_epoch", 0)
        best_loss         = resume_state.get("best_loss", np.inf)
        best_epoch        = resume_state.get("best_epoch", -1)
        epochs_no_improve = resume_state.get("epochs_no_improve", 0)
        print(f"[CoGAN] Resume da epoch {start_epoch}, best_loss={best_loss:.6f}, best_epoch={best_epoch}")
    else:
        start_epoch       = 0
        best_loss         = np.inf
        best_epoch        = -1
        epochs_no_improve = 0
        print("[CoGAN] Training da zero (nessun resume).")
    ...
    ckpt_G_path = os.path.join(model_fold_dir, cfg_trainer["CHECKPOINT_G"])
    ckpt_D_path = os.path.join(model_fold_dir, cfg_trainer["CHECKPOINT_D"])
    ...
    for epoch in range(start_epoch, max_epochs):
        ...
        if epoch > warm_up:
            if val_loss < best_loss:
                ...
                save_cogan_checkpoint_D(ckpt_D_path, D, opt_D)
    ...
    # ------------------------ FINE TRAINING ------------------------
    print("Training CoGAN terminato.")
    ...
    save_cogan_checkpoint_G(
        ckpt_G_path,
        G, opt_G,
        epoch=best_epoch if best_epoch >= 0 else (max_epochs - 1),
        best_loss=best_loss,
        best_epoch=best_epoch,
        epochs_no_improve=epochs_no_improve,
